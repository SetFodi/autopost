...
export function generateStaticParams() {
  return marketingGuides.map((guide) => ({ slug: guide.slug }))
}
...
export default async function GuidePage({
  ...
  const relatedGuides = marketingGuides.filter(
    (item) => item.slug !== guide.slug,
  )

  return (
    <MarketingPageShell path={`/guides/${guide.slug}`}>
      <JsonLd
        data={{
          ...
          author: { '@type': 'Organization', name: 'AutoPost' },
          publisher: {
            '@type': 'Organization',
            name: 'AutoPost',
      ...
      <article>
        <header className="grain relative overflow-hidden border-b border-white/[0.07] py-14 sm:py-20 lg:py-24">
          ...
          <div className="site-container relative max-w-5xl">
            ...
            >
              <ArrowLeft aria-hidden="true" className="size-3.5" /> ყველა
              გზამკვლევი
            </Link>
            <p className="section-kicker mt-10">{guide.eyebrow}</p>
            <h1 className="font-display text-ivory text-[clamp(2.5rem,7vw,5.8rem)] leading-[1.01] font-extrabold tracking-[-0.06em] [overflow-wrap:anywhere]">
              {guide.title}
            </h1>
            <div className="text-ivory/42 mt-7 flex flex-wrap items-center gap-4 border-t border-white/10 pt-5 text-xs">
              <span className="inline-flex items-center gap-2">
                ...
                {guide.readingMinutes} წუთი
              </span>
              <span aria-hidden="true">·</span>
              <time dateTime={MARKETING_CONTENT_UPDATED_AT}>
                განახლებულია 14 ივლისს, 2026
              </time>
            </div>
          </div>
        </header>
      ...
      <section className="border-y border-white/[0.07] bg-[#100e0c] py-14 sm:py-20">
        <div className="site-container">
          <p className="section-kicker">შემდეგი საკითხავი</p>
      ...
      <MarketingCtaBand source={`guide_${guide.slug}`} />
    </MarketingPageShell>
  )
}
